...
export default function Dashboard() {
  ...
  const groupedDevices = useMemo(() => {
    // '전체' 탭이 아닐 때는 그룹화를 적용하지 않음
    if (activeTab !== 'all') return null;

    return filteredDevices.reduce((acc, [name, device]) => {
      const category = device.category || '기타'; // 카테고리 없는 경우 '기타'로
      if (!acc[category]) {
        acc[category] = [];
      }
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="tabs">
        ...
        <button onClick={() => setActiveTab('operating')} className={activeTab === 'operating' ? 'active' : ''}>동작중</button>
        <button onClick={() => setActiveTab('waiting')} className={activeTab === 'waiting' ? 'active' : ''}>대기중</button>
        <button onClick={() => setActiveTab('completed')} className={activeTab === 'completed' ? 'active' : ''}>완료</button>
      </div>
...
const PowerButton = ({ name, data, onControl }) => (
  <button 
      ...
      title={data.power === 'on' ? '전원 끄기' : '전원 켜기'} // Add a tooltip for accessibility
  >
...
const CycleDeviceCard = ({ name, data, onSimulate, onControl }) => {
  ...
  let statusText = '대기';
  if (isOperating) {
    statusText = type === 'robot_vacuum' ? '청소중' : '동작중';
  } else if (isCompleted) {
    statusText = '완료';
  ...
  let buttonText = '동작 시작';
  if (isOperating) {
    buttonText = '동작 취소';
  } else if (isCompleted) {
    buttonText = '완료 확인';
  } else if (type === 'robot_vacuum') {
    buttonText = '청소 시작';
  }
        ...
        <div className="device-status">
          ...
          {courses && (
            <div className="control-group">
              <label>코스 선택:</label>
              <select value={course} onChange={(e) => onControl(name, 'course', e.target.value)} disabled={!isWaiting}>
