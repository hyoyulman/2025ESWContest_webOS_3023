        ...
        master_devices = [
            ...
            },
            {
                ...
                "default_settings": {
                    ...
                    "temperature": 180,
                    "mode": "convection",
                    "modes": ["convection", "grill", "steam", "air_fry"],
                    "timer": 0
                }
            },
            {
...
def control_device(user_id, device_name, command, value=None):
    ...
    if command == "power" and value in ["on", "off"]:
        if device["power"] != value:
            ...
            update_fields["power_on_timestamp"] = datetime.datetime.now(KST) if value == "on" else None
    
    # 온도 제어
    elif command == "temperature" and value is not None:
        if "temperature" in device:
            update_fields["temperature"] = value
        elif "fridge_temp" in device:  # 냉장고
            update_fields["fridge_temp"] = value
    
    # 모드 제어
    elif command == "mode" and value:
        if "modes" in device and value in device["modes"]:
            update_fields["mode"] = value
    
    # 코스 제어
    elif command == "course" and value:
        if "courses" in device and value in device["courses"]:
    ...
    # 풍속 제어
    elif command == "fan_speed" and value:
        if "fan_speeds" in device and value in device["fan_speeds"]:
            update_fields["fan_speed"] = value
        elif "fan_speed" in device:
            update_fields["fan_speed"] = value
    
    if update_fields:
        mongo.db.user_LG_devices.update_one(
            {"_id": device_name, "userId": user_id},
            {"$set": update_fields}
        )
        quests_service.update_quest_progress(user_id, device_name)
    
    return get_device_status(user_id, device_name)


def simulate_device_usage(user_id, device_name, start_time_iso=None):
    """가전 사용 시뮬레이션 (시작/완료 토글) - 여기가 존나 문제네"""
    device = mongo.db.user_LG_devices.find_one({"_id": device_name, "userId": user_id})
    if not device:
        raise ValueError(f"가전 '{device_name}'을(를) 찾을 수 없습니다.")
    
    if start_time_iso:
        start_time_kst = datetime.datetime.strptime(
    ...
    # 사이클 기반 가전 (세탁기, 건조기, 식기세척기, 스타일러 등)
    if device_type in ["washer", "dryer", "dishwasher", "styler", "shoe_care"]:
        current_status = device.get("status", "waiting")
        
        if current_status == "waiting":
            # 시작
            update_fields["status"] = "running"
            update_fields["power"] = "on"
            update_fields["cycle_start_timestamp"] = start_time_kst
            update_fields["power_on_timestamp"] = start_time_kst
            update_fields["remaining_time"] = device.get("total_time", 60)
        
        elif current_status == "running":
            # 완료
            update_fields["status"] = "completed"
            update_fields["cycle_start_timestamp"] = None
            update_fields["remaining_time"] = 0
            update_fields["power"] = "off"
    ...
    # 로봇청소기
    elif device_type in ["robot_vacuum"]:
        current_status = device.get("status", "docked")
        
        if current_status in ["docked", "completed"]:
            update_fields["status"] = "cleaning"
            update_fields["power"] = "on"
            update_fields["cycle_start_timestamp"] = start_time_kst
            update_fields["last_run_timestamp"] = start_time_kst
            update_fields["remaining_time"] = device.get("total_time", 60)
        
        elif current_status == "cleaning":
            update_fields["status"] = "docked"
            update_fields["power"] = "off"
            update_fields["cycle_start_timestamp"] = None
            update_fields["remaining_time"] = 0
            mongo.db.user_LG_devices.update_one(
    ...
    # 토글형 가전 (TV, 에어컨, 공기청정기 등)
    elif device_type in ["tv", "air_conditioner", "air_purifier", "refrigerator", "aero_tower", "dehumidifier", "oven"]:
        is_on = device["power"] == "on"
        update_fields["power"] = "off" if is_on else "on"
        update_fields["power_on_timestamp"] = None if is_on else start_time_kst
